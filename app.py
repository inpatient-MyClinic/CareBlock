"""
CareBlock Clinical Operations — Backend Server v3.0
...
# Railway gives us a /app working directory — use absolute path
...
# ── Run init at startup (works for both gunicorn and direct python) ──
try:
    init_db()
    print(f"[CareBlock] DB ready: {DB}")
except Exception as e:
    print(f"[CareBlock] DB init error: {e}")
...
@app.route('/')
def index():
    f = os.path.join(BASE_DIR, 'clinical_portal.html')
    if os.path.exists(f):
        return send_file(f)
    return 'clinical_portal.html not found', 404

@app.route('/clinical_portal.html')
def portal(): return index()
...
def get_requests():
    try:
        rows = get_db().execute(
            'SELECT data FROM requests ORDER BY id').fetchall()
...
def save_requests():
    try:
        ...
        for item in data:
            db.execute(
                'INSERT INTO requests (data,created_at,updated_at) VALUES(?,?,?)',
    ...
    try:
        ...
        for row in db.execute('SELECT rowid, data FROM requests').fetchall():
            ...
            if int(item.get('id', -1)) == rid:
                db.execute(
                    'UPDATE requests SET data=?,updated_at=? WHERE rowid=?',
...
def get_backups():
    try:
        rows = get_db().execute(
            'SELECT id,label,created_at FROM backups '
...
def create_backup():
    try:
        ...
        if count >= 5:
            oldest = db.execute(
                'SELECT id FROM backups ORDER BY id LIMIT 1').fetchone()
            if oldest:
                db.execute('DELETE FROM backups WHERE id=?', (oldest['id'],))
        db.execute(
            'INSERT INTO backups(label,data,created_at) VALUES(?,?,?)',
...
def get_backup(bk_id):
    try:
        row = get_db().execute(
            'SELECT data FROM backups WHERE id=?', (bk_id,)).fetchone()
        if row:
...
if __name__ == '__main__':
    ...
    print(f'\n CareBlock Clinical Operations v3.0')
    print(f' Local:  http://localhost:{port}')
    print(f' Network: http://{ip}:{port}')
    print(f' Doctor:  http://{ip}:{port}/?role=doctor')
    print(f' Team:    http://{ip}:{port}/?role=team\n')
